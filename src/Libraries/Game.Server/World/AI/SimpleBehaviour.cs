...
public class SimpleBehaviour : IBehaviour
{
    ...
    private void MoveTo(IEntity target, ServerTimestamp now)
    {
        ...
        if (_entity is MonsterEntity { Rank: < EMonsterLevel.BOSS } &&
            ShouldChangeAttackPosition(directionLength, now))
        {
            if (TryChangeAttackPosition(target, directionLength, minDistance, now))
            {
                return;
            }
        }
    ...
    public void Update(in TickContext ctx)
    {
